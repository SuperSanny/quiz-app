...
export default function QuizPage() {
  ...
  const [currentQuestion, setCurrentQuestion] =
    useState<CurrentQuestionDisplay | null>(null);
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 404 && isActive) {
            ...
            toast({
              title: "Error",
              description: "Quiz session not found or ended unexpectedly.",
              variant: "destructive",
            });
          ...
          // Add check for 500 specifically if needed
            ...
            console.error(
              "Server error (500) fetching quiz state. Check server logs."
            );
            toast({
              title: "Server Error",
              description:
                "Could not fetch quiz status. Please check server logs or retry later.",
              variant: "destructive",
            });
  ...
  useEffect(() => {
    ...
    if (quizState.quizSessionId && userSessionId !== quizState.quizSessionId) {
      ...
      toast({
        ...
        description:
          "You joined a different quiz session than the current one. Please rejoin.",
        variant: "destructive",
      });
    ...
    if (isActive && newIndex !== currentQuestionIndexRef.current) {
      ...
      // Index 0-9: Fetch standard question
        ...
        fetch(
          ...
          .then((res) => {
            if (!res.ok) {
              throw new Error(`Failed to fetch question ${newIndex + 1}`);
            }
            return res.json();
          })
          .then((questionData: CurrentQuestionDisplay) => {
            if (questionData.isFinished) {
              // Check if API returned finished state
              setStage("finished");
            } else {
      ...
      // Index 10: Fetch bonus question
        ...
        fetch(
          ...
          .then((res) => {
            if (!res.ok) {
              throw new Error(`Failed to fetch bonus question`);
            }
            return res.json();
          })
          .then((questionData: CurrentQuestionDisplay) => {
            if (questionData.isFinished) {
              // Handle case where bonus was skipped and quiz ended
              setStage("finished");
              ...
              console.error(
                "Expected bonus question data but got something else:",
                questionData
              );
              setStage("error");
            }
          ...
          .catch((err) => {
            console.error(err);
            toast({
              title: "Error loading bonus question",
              description: err.message,
              variant: "destructive",
            });
            setStage("error");
  ...
  const handleJoinQuiz = async () => {
    ...
    try {
      ...
      if (!stateRes.ok) {
        throw new Error("Failed to get initial quiz state");
      }
      const stateData: QuizStateData = await stateRes.json();

      // Check if session matches (important if admin restarts quickly)
      if (
        stateData.quizSessionId &&
        data.quizSessionId !== stateData.quizSessionId
      ) {
        toast({
          title: "Quiz Restarted",
          description:
            "The admin started a new quiz while you were joining. Please join again.",
          variant: "destructive",
        });
        setUserInfo(null);
        setStage("initial");
  ...
  const handleSubmitBonusAnswer = async () => {
    ...
    try {
      ...
      toast({
        title: "Error submitting bonus answer",
        description: error.message,
        variant: "destructive",
      });
    ...
    <Card className="w-full max-w-md text-center border-destructive">
      ...
      <CardContent className="flex flex-col items-center space-y-4">
        <p className="text-destructive-foreground bg-destructive p-3 rounded-md mb-4">
          {userInfo?.quizSessionId &&
          quizState?.quizSessionId &&
          userInfo.quizSessionId !== quizState.quizSessionId
