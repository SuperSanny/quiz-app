...
export default function AdminPage() {
  ...
  const [actionLoading, setActionLoading] = useState<
    "start" | "next" | "end" | null
  >(null); // Track which action is loading
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 401 && isMounted) {
            // Handle potential auth issues during polling
            toast({
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const expectedPassword =
      process.env.NEXT_PUBLIC_ADMIN_PASSWORD || "password"; // **INSECURE - FOR LOCAL DEMO ONLY**

    if (password === expectedPassword) {
      setIsAuthenticated(true);
      toast({
        title:
          "Password accepted (Client-side check). Actions will verify on backend.",
      });
    } else {
      toast({
        title: "Authentication failed (Client-side check)",
        variant: "destructive",
      });
  ...
  const handleAdminAction = async (action: "start" | "next" | "end") => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          toast({
            title: "Authentication Failed (Backend)",
            description: "Your admin password might be incorrect or expired.",
            variant: "destructive",
          });
      ...
      switch (action) {
        case "start":
          ...
          toastDescription = `Session ID: ${
            data.quizSessionId
          }. Bonus Question: ${
            bonusQuestionText.trim() ? "Included" : "Skipped"
          }`;
          setQuizState({
            isQuizActive: true,
        ...
        case "next":
          ...
          const nextIndexDisplay =
            data.newIndex === TOTAL_STANDARD_QUESTIONS
              ? "Bonus Question"
              : `Question ${data.newIndex + 1}`;
        ...
        case "end":
          ...
          toastDescription = `Session ${
            data.quizSessionId || quizState?.quizSessionId || "N/A"
          } is now inactive.`;
          if (quizState) {
  ...
  const canAdvance =
    ...
    quizState.currentQuestionIndex < TOTAL_STANDARD_QUESTIONS;
  ...
  if (quizState) {
    ...
    if (quizState.isQuizActive) {
      ...
      if (currentIndex >= 0 && currentIndex < TOTAL_STANDARD_QUESTIONS) {
        currentStepDisplay = `Question ${
          currentIndex + 1
        } of ${TOTAL_STANDARD_QUESTIONS}`;
      } else if (currentIndex === TOTAL_STANDARD_QUESTIONS) {
        currentStepDisplay = "Bonus Question Stage";
      } else if (currentIndex === -1) {
        statusMessage =
          "Quiz active, but waiting for first question. Press Start Quiz.";
        currentStepDisplay = "Pre-Start";
      } else {
        // Index > TOTAL_STANDARD_QUESTIONS (should not happen if 'next' logic is correct)
    ...
    <div className="container mx-auto p-4 py-8">
      <Card className="max-w-lg mx-auto shadow-xl">
        <CardHeader>
          ...
          <CardDescription>
            <span className="block font-medium">Session ID:</span>{" "}
            {quizState?.quizSessionId || "N/A"} <br />
            <span className="block font-medium mt-1">Status:</span>{" "}
            {statusMessage} {currentStepDisplay && `(${currentStepDisplay})`}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col space-y-4">
          ...
          >
            ...
            {quizState?.currentQuestionIndex === TOTAL_STANDARD_QUESTIONS - 1
        ...
        <CardFooter>
          <p className="text-xs text-muted-foreground">
            Control the quiz flow. Bonus question is shown after question{" "}
            {TOTAL_STANDARD_QUESTIONS}. Ending the quiz makes it inactive. State
            updates every {POLLING_INTERVAL / 1000}s.
          </p>
        </CardFooter>
      </Card>
    </div>
